...
class ShortFStrategy(IStrategy):
    """
    做空F（vps_sf）

    日线条件：收盘价不在 MA5/MA25 之下（close >= lower_ma）
    信号：收盘同时跌破5分钟 MA5 和 MA25，同时自上一次MA5/MA25交叉以来
          的最高收开盘价与当前较小均线的差值 > 800点
          当前K线成交额 < 1000万 USDC 且实体 < 100点（弱势跌破）
    入场：信号K线下一根开盘立即入场
    止损：sig_open + sig_range
    止盈：sig_open - sig_range（sig_range = 记录高点 - sig_open）
    扩展：触发止盈时放量阴线(>=3倍20日均量) -> sig_open - 3*sig_range
    """
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # 自上次交叉以来的最高收开盘价与当前较小均线的差值 > 800
        range_ok = (dataframe['high_since_cross'] - dataframe['ma_min']) > 800

        # 当前K线量能 < 1000万 且 实体 < 100点（弱势跌破，可能是假突破）
        vol = dataframe['volume'] * dataframe['close']
        weak_candle = (vol >= 5_000_000) & (vol < 10_000_000) & ((dataframe['close'] - dataframe['open']).abs() < 100)

        condition = not_below_both & below_both_ma & range_ok & weak_candle

        for idx in condition[condition].index:
            pos = dataframe.index.get_loc(idx)
            ma_min_val = dataframe.iloc[pos]['ma_min']
    ...
    def custom_stoploss(self, pair: str, trade: 'Trade', current_time: datetime,
        ...
        if trade.enter_tag and trade.enter_tag.startswith('vps_sf'):
    ...
    def custom_exit(self, pair: str, trade: 'Trade', current_time: datetime,
        ...
        if not trade.enter_tag or not trade.enter_tag.startswith('vps_sf'):
            return None
    ...
    def confirm_trade_exit(self, pair: str, trade: Trade, order_type: str, amount: float,
        ...
        if trade.enter_tag and trade.enter_tag.startswith('vps_sf'):
            ...
            if len(parts) >= 3:
                ...
                print(f"  [成交] {dir_cn}F 信号开盘={sig_open:.1f} 振幅={sig_range:.1f}点 "
